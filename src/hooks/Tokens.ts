...
export function useUnsupportedTokens(): { [address: string]: Token } {
  ...
  const l2InferredBlockedTokens: typeof unsupportedTokens = useMemo(() => {
    ...
    return list.tokens.reduce((acc, tokenInfo) => {
      ...
      if (
        bridgeInfo &&
        bridgeInfo[SupportedChainId.ETHPOW] &&
        bridgeInfo[SupportedChainId.ETHPOW].tokenAddress &&
        unsupportedSet.has(bridgeInfo[SupportedChainId.ETHPOW].tokenAddress)
      ) {
        const address = bridgeInfo[SupportedChainId.ETHPOW].tokenAddress
        // don't rely on decimals--it's possible that a token could be bridged w/ different decimals on the L2
        return { ...acc, [address]: new Token(SupportedChainId.ETHPOW, address, tokenInfo.decimals) }
      }
      return acc
    }, {})
